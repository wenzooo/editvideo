...
def run_transcribe(job_id: str, video_id: str) -> None:
    from .services.transcribe import captions_with_words, transcribe_words  # lazy
    ...
    words, fallback = transcribe_words(
        path, duration, progress_cb=lambda p: _set_progress(job_id, p))

    if wants_retakes:  # doppioni/ripartenze (dalle parole trascritte)
        try:
    ...
    captions = captions_with_words(words, fallback)
